...
const  Home = () => {
    ...
    const API_KEY = "eb0bde145b49df90d2d38d0db5fce481";
    ...
    const fetchMovies = async (event) => {
        ...
        let randomPosition = Math.floor(Math.random() * data.results.length ); 
            
         setMovie(data.results[randomPosition]);
            if (data.results.length) {
                await fetchMovie(data.results[randomPosition].id);
            }
    }
